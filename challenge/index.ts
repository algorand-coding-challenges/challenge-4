...
const dispenser = await algokit.getLocalNetDispenserAccount(algodClient);

const sender = new SigningAccount(dispenser, dispenser.addr);
...
atc.addTransaction({ txn: ptxn1, signer: sender.signer });
atc.addTransaction({ txn: ptxn2, signer: sender.signer });
